...
export function calculateAutoOt(input: RoiInput) {
...
export function getUtilStatus(percent: number): Pick<RoiResult, "status" | "statusColor"> {
  if (percent < 60) return { status: "Excellent", statusColor: "green" };
  if (percent < 80) return { status: "Normal", statusColor: "yellow" };
  if (percent <= 100) return { status: "Busy", statusColor: "orange" };
  return { status: "Overloaded", statusColor: "red" };
}

export function calculateRoi(rawInput: RoiInput): RoiResult {
  ...
  const oldDep =
    nonNegative(rawInput.alcBuy) / Math.max(1, nonNegative(rawInput.alcLife)) +
  ...
  const roi = newGrand > 0 ? (annualSave / newGrand) * 100 : 0;
