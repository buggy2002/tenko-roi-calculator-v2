...
import type { Language, PresetKey, RoiInput } from '@/utils/roi/types'
import {
  createScenario,
  deleteScenarioRequest,
  hasScenarioApiConfig,
  listScenarios,
  updateScenario,
} from '@/services/roi-scenarios'
import type {
...
function getDefaultScenarioName(presetKey: PresetKey) {
  return roiPresets[presetKey].name
}

function resolvePresetKey(value: PresetKey | null | undefined): PresetKey {
  return value && value in roiPresets ? value : 'default'
}

function inferFactorChoice(value: number): StoredScenario['factorChoice'] {
  const fixed = value.toFixed(2)
  return ['1.20', '1.25', '1.35'].includes(fixed) ? fixed : 'custom'
}

function inferAutoOtEnabled(presetKey: PresetKey) {
  return roiPresets[presetKey].autoOT
}

function normalizeStoredScenario(raw: Partial<StoredScenario>): StoredScenario | null {
  if (
...
function toStoredScenarioFromRecord(record: RoiScenarioRecord, fallbackLocalId?: string): StoredScenario {
...
function mergeScenarioCollections(localItems: StoredScenario[], remoteItems: StoredScenario[]) {
...
export const useRoiStore = defineStore('roi', () => {
  ...
  const presetKey = ref<PresetKey>('default')
  const input = ref<RoiInput>({
    ...defaultInput,
  ...
  const currentLocalId = ref<string | null>(null)
  ...
  const renamingLocalId = ref<string | null>(null)
  ...
  const scenarioGroups = computed<ScenarioListGroup[]>(() => {
    ...
    const groups: ScenarioListGroup[] = []
  ...
  function setEditorFromScenario(scenario: StoredScenario) {
  ...
  function syncSavedScenario(scenario: StoredScenario) {
    savedScenarios.value = mergeScenarioCollections(
      savedScenarios.value.filter(item => item.localId !== scenario.localId),
      [scenario],
    )
  }
  ...
  function applyInput(next: RoiInput, changedKey?: keyof RoiInput, forceAuto = false) {
  ...
  function updateInput(key: keyof RoiInput, value: number) {
  ...
  function activatePresetTab(key: PresetKey) {
    currentLocalId.value = null
    currentRemoteId.value = null
  ...
  function openScenario(scenario: StoredScenario) {
  ...
  function closeScenarioTab(localId: string) {
  ...
  function duplicateScenario(scenario: StoredScenario) {
    const nextScenario: StoredScenario = {
      ...scenario,
  ...
  function startRenameScenario(scenario: StoredScenario) {
  ...
  async function submitRenameScenario(scenario: StoredScenario) {
    const nextName = renameDraft.value.trim()
    if (!nextName)
      return 'empty'

    const updatedScenario: StoredScenario = {
      ...scenario,
      name: nextName,
      savedAt: new Date().toISOString(),
    }
  ...
  function toCreatePayload(): CreateRoiScenarioInput {
    return {
  ...
  function toUpdatePayload(): UpdateRoiScenarioInput {
    return toCreatePayload()
  }

  async function saveScenario() {
    const nextLocalId = currentLocalId.value ?? createLocalId()
    const baseScenario: StoredScenario = {
      localId: nextLocalId,
  ...
  function hydrate() {
    ...
    if (!hasRestoredEditor.value && savedScenarios.value.length > 0) {
      const targetScenario
        = (currentLocalId.value ? savedScenarios.value.find(item => item.localId === currentLocalId.value) : undefined)
